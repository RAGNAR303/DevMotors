...
export async function getDataHome() {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/${process.env.COSMIC_BUCKET_SLUG}/objects/${process.env.COSMIC_BUCKET_ID}?pretty=true&read_key=${process.env.COSMIC_READ_KEY}&depth=1&props=slug,title,metadata,type`,
      { next: { revalidate: 120 } },
    );
...
export async function getSubMenu() {
  try {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/devmotors-production-0db0ad50-25ef-11f1-aef4-37efac6b10eb/objects?pretty=true&query=%7B%22type%22:%22pages%22%7D&limit=10&skip=0&read_key=TDvTVz5lJdnnIUaByQRzB5AhnORnYqo6ALpAFe3fDSbfuSgWzc&depth=1&props=slug,title,`,
      { next: { revalidate: 120 } },
    );
...
export async function getPageBySlug(slug: string) {
  ...
  try {
    // await delay(1000); //simulação de um delay na página 
